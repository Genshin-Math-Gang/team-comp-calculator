...
namespace Tcc.events
{
    ...
    {
        public Hit(
            double timestamp, double mvs, Func<double, SecondPassStatsPage> stats,
            ...
            (world) => world.CalculateDamage(
                timestamp, mvs, stats(timestamp), 
                unit, type, hitType, description), description
        ) {
        }
        // TODO: scuffed, make sure that the write constructor is called
        public Hit(
            ...
            base(
                ...
                // TODO reaction is scuffed and only used for transformative reactions so i want to remove it
                (world) => world.CalculateDamage(
                    timestamp, unit.StartingAbilityStats[type].GetMotionValue(mvIndex), stats(timestamp), 
                    unit, type, hitType, description), description
            ) {
        }
