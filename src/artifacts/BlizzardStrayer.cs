...
namespace Tcc.Buffs.Artifacts
{
    public class BlizzardStrayer: ArtifactSet
    {
        ...
        static readonly FirstPassModifier MODIFIER_2PC = _ => new StatsPage(Stats.Stats.CryoDamageBonus, 0.15);

        static readonly EnemyBasedModifier MODIFIER_4PC = (data) =>
        {
            if (data.enemy.HasAura(Aura.FROZEN)) return new StatsPage(Stats.Stats.CritRate, 0.4);
            if (data.enemy.HasAura(Aura.CRYO)) return new StatsPage(Stats.Stats.CritRate, 0.4);
